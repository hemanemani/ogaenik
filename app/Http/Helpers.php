...
use App\Models\SellerBusinessSetting;
...
function getShippingCostdsf($index){
    $admin_products = array();
    $seller_products = array();
    $calculate_shipping = 0;
...
if (!function_exists('uploaded_alt')) {
    function uploaded_alt($id)
    {
        if (($asset = \App\Models\Upload::find($id)) != null) {
...
if (! function_exists('my_asset_image')) {
    /**
     ...
     * @return string
     */
    function my_asset_image($path, $secure = null)
    {
...
if (!function_exists('getFileBaseImageURL')) {
function getFileBaseImageURL()
    {
		
       return config('app.cdn_subdomain');
    }
}
...
if (!function_exists('seller_get_setting')) {
    function seller_get_setting($key, $default = null)
    {
        $setting = SellerBusinessSetting::where('type', $key)->first();
        return $setting == null ? $default : $setting->value;
    }
}
